...
def fit_all_stage1_models(
    ...
    df_y_train = np.asarray(y_train).astype(float)
    df_y_val = np.asarray(y_val).astype(float)

    # --- Tabular baselines
    tabular_models, df_tabular_results = fit_tabular_baselines(
        X_train=X_train,
        X_val=X_val,
        y_train=df_y_train,
        y_val=df_y_val,
...
def predict_all_stage1_models(
    ...
    ----------
    trained_bundle : dict
        Output of fit_all_stage1_models().
    X_input : pd.DataFrame
    ...
    -------
    dict[str, np.ndarray]
        Prediction arrays keyed by model name.
    """
...
def save_stage1_condition_artifacts(
    ...
    # Save tabular weak-distillation results
